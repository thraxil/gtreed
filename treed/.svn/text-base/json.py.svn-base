...
from treed.model import User, Group, Permission, Item, ListItem
...
@jsonify.when('isinstance(obj, Item)')
def jsonify_item(obj):
    result = jsonify_sqlobject(obj)
    result["children"] = [jsonify_item(li.child) for li in obj.children]
    return result

@jsonify.when('isinstance(obj, ListItem)')
def jsonify_listitem(obj):
    return jsonify_sqlobject(obj.child)
