...
__revision__ = "$Rev: 470 $"
__date__ = "$Date: 2007-01-12 20:25:43 -0500 (Fri, 12 Jan 2007) $"
...
def raise_template_error(module=None, filename=None, encoding=None):
    ...
    if filename and filename != '<string>':
        ...
        lineno = getattr(exc_value, 'lineno',
            extract_tb(sys.exc_info()[2])[-1][1])
        if lineno:
            location = error_location(filename, encoding, lineno=lineno)
            if location:
